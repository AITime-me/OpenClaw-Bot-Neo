...
import {
  DEFAULT_INFRASTRUCTURE_CONNECTOR_SIMULATION,
  infrastructureMutationOutcomeUnknownError,
  type InfrastructureConnectorSimulation,
} from './infrastructure-connector-simulation.js';
...
const ANSI_SECRET_FIXTURE = ['\x1b[31mpassword=secret123\x1b[0m', 'ok'];

export const createInfrastructureConnector = (deps: InfrastructureConnectorDeps): Connector => {
  const simulation = { ...DEFAULT_INFRASTRUCTURE_CONNECTOR_SIMULATION, ...deps.simulation };
  ...
  return {
    ...
    async execute(request: ConnectorExecuteRequest): Promise<ConnectorExecutionResult> {
      ...
      if (toolId === 'infrastructure.server.inspect') {
        if (simulation.serverInspect === 'unavailable')
          return {
            ok: false,
      ...
      if (toolId === 'infrastructure.server.resources.read') {
        if (simulation.serverResources === 'unavailable')
          return {
            ok: false,
            error: { code: 'unavailable', reason: 'Unavailable.', category: 'internal' },
          };
        if (simulation.serverResources === 'invalid')
          return { ok: true, output: { unexpected: true } };
        const pressure = simulation.serverResources === 'pressure';
        return {
          ok: true,
          output: {
      ...
      if (toolId === 'infrastructure.service.status.read') {
        if (simulation.serviceStatus === 'unreachable')
          return {
            ok: false,
        ...
        return {
          ...
          output: {
            ...
            activeState: simulation.serviceStatus === 'failed' ? 'stopped' : 'running',
          },
        };
      }
      ...
      if (toolId === 'infrastructure.service.logs.read') {
        const raw =
          simulation.logFixture === 'secrets'
            ? ['password=secret123', 'started']
            : simulation.logFixture === 'multiline-key'
              ? PEM_MULTILINE_FIXTURE
              : simulation.logFixture === 'ansi'
                ? ANSI_SECRET_FIXTURE
                : Array.from(
                    { length: simulation.logFixture === 'truncated' ? 50 : 2 },
                    (_, i) => `line-${String(i)}`,
                  );
      ...
      if (toolId === 'infrastructure.service.restart') {
        if (simulation.restartMutation === 'outcome-unknown')
          return infrastructureMutationOutcomeUnknownError();
        if (simulation.restartMutation === 'cancel') {
          try {
      ...
      if (toolId === 'infrastructure.release.deploy') {
        if (simulation.deployMutation === 'outcome-unknown')
          return infrastructureMutationOutcomeUnknownError();
        return {
          ok: true,
          output: { deployed: true, releaseId: input.releaseId as string },
        };
      }

      if (toolId === 'infrastructure.release.rollback') {
        if (simulation.rollbackMutation === 'outcome-unknown')
          return infrastructureMutationOutcomeUnknownError();
        return {
          ok: true,
          output: { rolledBack: true, releaseId: input.releaseId as string },
        };
      }

      if (toolId === 'infrastructure.server.reboot') {
        if (simulation.rebootMutation === 'outcome-unknown')
          return infrastructureMutationOutcomeUnknownError();
        return { ok: true, output: { rebooted: true } };
      }
      ...
      return {
        ...
        error: {
          code: 'unavailable',
          reason: 'Unknown infrastructure tool.',
          category: 'internal',
        },
      };
    },
