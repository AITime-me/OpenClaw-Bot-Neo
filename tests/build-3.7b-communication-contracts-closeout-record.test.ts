...
const BUILD_BASE = '9a8f6771be345bfd50bae7b6838fe720342a3751';
const BUILD_SUBJECT = 'feat(communication): add Build 3.7B offline contracts';

const CLOSEOUT_REL = 'docs/validation/build-3.7b-communication-contracts-closeout.md';
const CLOSEOUT_PATH = join(REPO_ROOT, CLOSEOUT_REL);
const TEST_REL = 'tests/build-3.7b-communication-contracts-closeout-record.test.ts';
...
const ALLOWED_PREFIXES = [
  ...
  'scripts/',
  'tests/communication/',
  'tests/fixtures/boundaries/communication-allowed/',
  ...
  'tests/fixtures/boundaries/forbidden-communication-principal-internal/',
] as const;
...
function parseCanonicalMarkers(record: string): Map<string, string> {
  const begin = 'BEGIN_BUILD_3_7B_MARKERS';
  const end = 'END_BUILD_3_7B_MARKERS';
...
describe('Build 3.7B communication contracts closeout record', () => {
  ...
  it('parses exactly one canonical marker block with exact keys and values', () => {
    ...
    expect(markers.get('NEXT_STAGE')).toBe('3.7C');
    expect(markers.get('OFFLINE_ONLY')).toBe('TRUE');
  });

  it('does not claim production readiness or security approval', () => {
    const markers = parseCanonicalMarkers(record);
  ...
  it('keeps E1/F blocked and next stage 3.7C', () => {
    const markers = parseCanonicalMarkers(record);
  ...
  it('limits Build 3.7B changed paths to the allowlist and forbids sensitive paths', () => {
    ...
    expect(all.includes('src/core/communication/index.ts')).toBe(false);
    expect(all.includes('src/core/communication/application')).toBe(false);
  });
