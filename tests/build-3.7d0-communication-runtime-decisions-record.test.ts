...
/** Unified Build 3.7D ships D0 contract delta + runtime in one commit. */
const BUILD_SUBJECT = 'feat(communication): add Build 3.7D offline runtime';

const CLOSEOUT_REL = 'docs/validation/build-3.7d0-communication-runtime-decisions.md';
const CLOSEOUT_PATH = join(REPO_ROOT, CLOSEOUT_REL);
const TEST_REL = 'tests/build-3.7d0-communication-runtime-decisions-record.test.ts';
...
const ALLOWED_EXACT = new Set([
  ...
  'tests/build-3.7d-communication-runtime-closeout-record.test.ts',
...
function parseCanonicalMarkers(record: string): Map<string, string> {
  const begin = 'BEGIN_BUILD_3_7D0_MARKERS';
  const end = 'END_BUILD_3_7D0_MARKERS';
...
describe('Build 3.7D0 communication runtime decisions record', () => {
  ...
  it('parses exactly one canonical marker block with exact keys and values', () => {
    ...
    expect(markers.get('BUILD_ID')).toBe('3.7D0');
    expect(markers.get('BUILD_KIND')).toBe('RUNTIME_DECISIONS_CONTRACT');
    expect(markers.get('EXECUTABLE_COMMUNICATION_RUNTIME')).toBe('ABSENT');
    expect(markers.get('FAIL_SAFE_NO_RESUME_RECOVERY')).toBe('REQUIRED');
    expect(markers.get('NEXT_STAGE')).toBe('3.7D_IMPLEMENTATION');
  });
  ...
  it('keeps E1/F blocked and does not claim production readiness', () => {
    ...
    expect(markers.get('IMPLEMENTATION_STATUS')).toBe('CONTRACTS_ONLY');
  });

  it('limits Build 3.7D0+3.7D changed paths to the allowlist', () => {
